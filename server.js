...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`SELECT * FROM [dbo].[Users] WHERE Username = @username AND PasswordHash = @password`);
    ...
    return res.json({ success: true, user: { id: user.Id, name: user.FullName, username: user.Username, email: user.Email, role: user.Role, isAdmin: user.IsAdmin === 1 } });
  } catch (error) {
